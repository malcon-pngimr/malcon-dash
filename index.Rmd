...
```{r}
hf_hc <- paste0(hf_type$n[2], " (", round(hf_type$n[2]/hf_total*100, 2), "%)")
...
```{r}
hf_ap <- paste0(hf_type$n[1], " (", round(hf_type$n[1]/hf_total*100, 2), "%)")
...
```{r}
hf_hosp <- paste0(hf_type$n[3], " (", round(hf_type$n[3]/hf_total*100, 2), "%)")
...
Note that the figures for **Hela** and **Jiwaka** are combined with those of **Southern Highlands** and **Western Highlands** because the `rnaturalearth` package only allows `20` provinces instead of `22`. 
